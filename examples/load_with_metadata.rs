use collectd_plugin::{
    collectd_plugin, ConfigItem, MetaValue, Plugin, PluginCapabilities, PluginManager,
...
impl Plugin for MyPlugin {
    ...
    fn read_values(&self) -> Result<(), Box<dyn error::Error>> {
        ...
        ValueListBuilder::new(Self::name(), "load")
            .values(&values)
            .metadata("environment", MetaValue::String("prod".to_string()))
            .metadata("version", MetaValue::UnsignedInt(5))
            .submit()?;
